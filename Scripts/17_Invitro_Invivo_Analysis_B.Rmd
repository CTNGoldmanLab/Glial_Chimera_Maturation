...
## Compute Log2FC separately because Seurat is wonky for SCENIC  values
transitionSCENICexpr <- DotPlot(transition, group.by = "stage", assay = "SCENIC", features = row.names(transition))$data
transitionSCENICexpr <- transitionSCENICexpr[,c(1,3,4)] %>% pivot_wider(id_cols = features.plot, names_from = id, values_from = avg.exp)
transitionSCENICexpr[,2:3] <- log2(transitionSCENICexpr[,2:3])
transitionSCENICexpr$log2FC <- transitionSCENICexpr$`In Vivo` - transitionSCENICexpr$`In Vitro - GPC Stage`

#transitionSCENICexpr$pseudoExpr <- log2(transitionSCENICexpr$avg.exp + 1/10000)
#transitionSCENICexpr$pseudoExpr <- log2(transitionSCENICexpr$avg.exp + 1/10000)


transitionSCENIC$log2FC <- plyr::mapvalues(transitionSCENIC$regulon, from = transitionSCENICexpr$features.plot, transitionSCENICexpr$log2FC)

transitionSCENIC$log2FC <- as.numeric(transitionSCENIC$log2FC)
...
sigRegulons <- sigRegulons[,c(1,3,4,9,10)]
...
sigRegulonsScatter <- sigRegulonsScatter[,c(1,3,4,5,10)]
